...
def plot_confusion_matrix(cm, label_encoder, savefig=False, filename="cm", imp=False, rf_model=None, feature_names=[]):
    ...
    if "False" in filename:
        plt.title("RF Without EMG Features")
    else:
        plt.title("RF With EMG Features")

    plt.tight_layout()
    if savefig:
        plt.savefig(f"./plots/confusion/{filename}.png", dpi=300)
        plt.close()
    else:
        plt.show()
    ...
    if imp:
        ...
        if "False" in filename:
            plt.title("Top 15 Features by Importance -- RF Without EMG Features")
        else:
            plt.title("Top 15 Features by Importance -- RF With EMG Features")
        plt.tight_layout()
        if savefig:
            plt.savefig(f"./plots/feature_importance/{filename}.png", dpi=300)
            plt.close()
        else:
            plt.show()
